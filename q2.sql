...
Create View Rides as 
	select Request.request_id as request_id, client_id, datetime
...
Create View OneToTenRides as
	...
	from client join Rides on client.client_id = rides.client_id
	where rides.datetime >= '2020-01-01' and rides.datetime < '2021-01-01'
...
Create View FiveHundo As 
	select client_id.client_id as client_id, sum(amount) as billed
	from client join (request join billed on billed.request_id = request.request_id) on client.client_id = request.client_id
	where request.datetime < '2020-01-01'
	group by client.client_id
	having sum(amount) >= 500;

-- clients who had fewer rides in 2021 than in 2020 
Create View FewerRides As 
	select twenty.client_id as client_id, twenty - twentyone as decline
	from 
		(select client_id, count(*) as numRides from rides where datetime >= '2020-01-01' and datetime < '2021-01-01') twenty 
		join 
		(select client_id, count(*) as numRides from rides where datetime >= '2021-01-01' and datetime < '2022-01-01') twentyone
...
INSERT INTO q2
	Select client_id, CONCAT(firstname, " ", surname) as name, Coalesce(email, "unknown") as email, billed, decline
